...
def UI():
    ...
    window.close()
    return [values[0], values[1], values[2], '' if GS_b else 's', 'v' if oneNote_on else '', 0 if langughe_on else 1]
...
def Advertise(thr):
    ...
    if exists(p):
        ...
        response = requests.get("https://github.com/gimdani/pdf-to-tiny-book/blob/main/%D7%A4%D7%A8%D7%A1%D7%95%D7%9E%D7%AA.jpg?raw=true")
    ...
    for inp in inputs[0]:
        ...
        for i in range(int(number_of_pages / notebook_len) + (number_of_pages % notebook_len > 0)):
            ...
            pdfbooklet_new.pdfbooklet(path + file_name + str(i + 1) + '.pdf',
                                      path + file_name + str(i + 1) + 'let.pdf', eng=eng)
        ...
        if pages_per_sheet > 2:
            ...
            pdfbooklet_new.pdfbooklet(path + file_name + '_odd_rotated.pdf', odd_path, 1, booklet=0, eng=eng)
            pdfbooklet_new.pdfbooklet(path + file_name + '_even_rotated.pdf', even_path, 1, booklet=0, eng=eng)
            ...
            if pages_per_sheet > 4:
                ...
                pdfbooklet_new.pdfbooklet(path + file_name + '_odd_rotated2.pdf', odd_path, booklet=0, eng=eng)
                pdfbooklet_new.pdfbooklet(path + file_name + '_even_rotated2.pdf', even_path, booklet=0, eng=(eng==0))
                ...
                if pages_per_sheet > 8:
                    ...
                    pdfbooklet_new.pdfbooklet(path + file_name + '_odd_rotated3.pdf', odd_path, booklet=0, eng=eng)
                    pdfbooklet_new.pdfbooklet(path + file_name + '_even_rotated3.pdf', even_path, booklet=0, eng=eng)
...
if __name__ == '__main__':
    ...
    window_end = Window("thank U", win_end_layout, size=(200, 60))
